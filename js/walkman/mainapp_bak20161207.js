...
////初始化主体页面
app.controller('mainController', function($scope,$http,$element,$rootScope,$state,$compile) {
	...
	$rootScope.GuideArr =[
	{guide:["<div class='mg_div'><div class='guide-2'></div><div class='guide-1' ng-click='guidePlayNext()'></div></div>","<div class='mg_div'><div class='guide-3'></div><div class='guide-4'></div><div class='guide-1' ng-click='guidePlayNext()'></div></div>"]},
	{guide:["<div class='mg_div'><div class='guide-5'></div><div class='guide-6'></div><div class='guide-1' ng-click='guidePlayNext()'></div></div>"]},
	{guide:[]}
	];
...
//  	}
    ...
    $rootScope.guidePlayNext = function(){
	    ...
	    switch (action) {
	        ...
	        case "pre":
	            ...
	            if (curMusic[2] < 0) {
	                ...
	                curMusic[2] = $rootScope.UnitTree[curMusic[0]].SubTree[curMusic[1]].length - 1;
	...
	if (localStorage.curTime && localStorage.curMusicNo) {
	...
	/////
	$rootScope.showTips = function(strtips){
    ...
    function updateMusicList(){
    	...
    	if(isDelCur){ 			
			...
			if(isDelUnt&&isDelModule)
			{
				...
				{
					//////当前单元超出module的单元索引
			        var maxuntilIndex = $rootScope.UnitTreeObj[curMusic[0]].SubTree.length - 1;
			        $rootScope.curMusicNo = curMusic[0] + "," + maxuntilIndex + "," + ($rootScope.UnitTreeObj[curMusic[0]].SubTree[maxuntilIndex].SubTree.length - 1);
					$rootScope.curMusicNo=GetcurMusicNo("next");
...
app.config(function($stateProvider,$urlRouterProvider){
	$urlRouterProvider.otherwise('/selbook');
	$stateProvider
	.state('selbook',{
		url:'/selbook',
